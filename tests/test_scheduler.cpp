...
struct Params
{
	...
	Params(int id, MSQueue<int> &q, std::atomic<int> &push_count, std::atomic<int> &pop_count, std::mutex &result_mutex, std::unordered_set<int> &results, std::mutex &print_mutex, int *completed_producer, int *completed_consumer) : id(id), q(q), push_count(push_count), pop_count(pop_count),
	...
	Params(int id, const Params &tmp) : id(id), q(tmp.q), push_count(tmp.push_count), pop_count(tmp.pop_count),
